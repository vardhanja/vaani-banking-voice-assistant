...
| `ai/orchestrator/supervisor.py` | Implements `HybridSupervisor` which validates input via guardrails, builds context, asks the router for an intent, executes the chosen specialist, sanitizes output via guardrails, and shapes the final response payload. |
...
## Message Lifecycle
1. **Input validation:** `HybridSupervisor` validates user input via `GuardrailService.validate_input()` before processing. Checks include content moderation, PII detection, prompt injection protection, and rate limiting. If blocked, returns refusal message immediately (skips LLM and agents).
2. **Context build:** `HybridSupervisor._build_context` converts `message_history` into LangChain `HumanMessage`/`AIMessage` objects, infers UPI mode (frontend flag > history text > `structured_data` hints), and creates a `ConversationState` instance.
3. **Intent classification:** `IntentRouter.assign_intent` sends the state dict through `classify_intent`. Any fields mutated by the classifier are merged back into the dataclass.
4. **Routing:** `IntentRouter.resolve_route` maps the high-level intent (`upi_payment`, `banking_operation`, `general_faq`, `greeting`, `feedback`, etc.) to a concrete specialist key.
...
6. **Output sanitization:** `_build_response` sanitizes the response via `GuardrailService.sanitize_output()` - redacts PII, normalizes refusals, and verifies language consistency.
7. **Response shaping:** `_build_response` extracts the final assistant message plus any structured payloads and timestamp so HTTP clients receive the same schema as before.
...
- **Input guardrails:** All user input is validated before processing (content moderation, PII detection, prompt injection protection). Malicious queries are blocked early, saving LLM tokens and preventing security issues.
- **Output guardrails:** All AI responses are sanitized before sending (PII redaction, language consistency, safety checks). Prevents accidental data leakage and ensures responses match requested language.
