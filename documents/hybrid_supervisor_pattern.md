...
- **Business logic reuse:** Banking/UPI/FAQ agents stay untouched. They keep returning the same state dictionary they always have.
...
4. **Specialist execution:** The supervisor calls the selected agent (`banking_agent`, `upi_agent`, `faq_agent`, `greeting_agent`, or `feedback_agent`). Returned state fields (messages, structured data, statements, UPI mode) are applied back onto the shared context.
...
- **Unknown intents:** Router falls back to `faq_agent` through `DEFAULT_ROUTE`, so user always receives a graceful response.
