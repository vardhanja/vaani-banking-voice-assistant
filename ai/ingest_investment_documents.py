# -*- coding: utf-8 -*-
"""
Investment Scheme Document Ingestion Script
Processes investment scheme PDFs and creates vector database for RAG
Run this script to populate the investment vector database with documents
"""
import sys
...
def main():
    """Main ingestion function for investment schemes"""
    print("=" * 60)
    print("INVESTMENT SCHEME DOCUMENTS INGESTION")
    print("=" * 60)
    
    # Initialize RAG service with explicit paths for investment schemes
    ...
    if not documents_path.exists():
        ...
        print("   Please ensure investment scheme PDFs are in backend/documents/investment_schemes/")
        print("   Run: python backend/documents/create_investment_scheme_docs.py")
        return 1
    
    pdf_files = list(documents_path.glob("*.pdf"))
    if not pdf_files:
        print(f"\n❌ ERROR: No PDF files found in {documents_path}")
        return 1
    ...
    try:
        ...
        # Test retrieval
        print(f"\n🔄 Testing retrieval...")
        test_query = "What is PPF interest rate?"
        ...
        if results:
            ...
            for i, doc in enumerate(results, 1):
                ...
                scheme_type = doc.metadata.get("scheme_type", "Unknown")
                preview = doc.page_content[:150].replace("\n", " ")
                print(f"   {i}. {source} ({scheme_type})")
        ...
        print("   You can now use RAG for investment scheme Q&A!")
